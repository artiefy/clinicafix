...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const schema = z.object({
      status: z.union([z.literal("Disponible"), z.literal("Limpieza"), z.literal("Ocupada")]),
    });
    ...
    if (hasAssigned && assignedPatient && assignedPatient.discharge_status !== "de alta") {
      if (status !== "Ocupada") {
        return NextResponse.json(
          { error: "No se puede cambiar el estado: la cama tiene un paciente activo. Marque al paciente como 'de alta' primero." },
          { status: 400 }
        );
      }
    }
    ...
    // liberar la relación (mejor práctica: pacientes API debería haber hecho bed_id = null; igual permitimos)
